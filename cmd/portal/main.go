...
func main() {
	...
	listen := cli.String("listen", env("PORTAL_LISTEN", ""), "Address to serve on; default :443, or :8080 with --insecure (env PORTAL_LISTEN)")
	...
	tlsCA := cli.String("tls-ca", os.Getenv("PORTAL_TLS_CA"), "CA bundle PEM to verify the hub (env PORTAL_TLS_CA)")
	...
	dial := []monolink.Option{monolink.WithLogger(log.NewLogLogger(handler, log.LevelWarn))}
	switch {
	case *tlsCert != "" && *tlsKey != "":
		cfg, err := monolink.LoadClientTLS(*tlsCert, *tlsKey, *tlsCA)
		if err != nil {
			log.Error("bus TLS configuration failed", "err", err)
			os.Exit(1)
		}
		dial = append(dial, monolink.WithTLS(cfg))
	case *tlsCert != "" || *tlsKey != "":
		log.Error("bus TLS incomplete: --tls-cert and --tls-key are both required")
		os.Exit(1)
	}
	...
	mux.Handle("/", http.FileServerFS(app))
